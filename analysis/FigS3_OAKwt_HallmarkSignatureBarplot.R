...
metadata <- read.table("../data/OAK_KRASwt_subtypes.tsv", sep="\t", header=TRUE, check.names=FALSE)
...
pdf("../figures/FigS3_OAKwt_HallmarkSignatureBarplots.pdf", width=4.2, height=7.5)
...
boxplot(scores ~ subtypes, horizontal=TRUE, col=colors, outline=FALSE,
				...
				xlab="Signature scores", yaxt="n", main="OAK - KRAS wildtype")
axis(side=2, at=seq(from=2, to=4*length(sets), by=4), labels=rep("", length(sets)), las=2, cex.axis=0.7)
text(x=1.7, y=seq(from=2, to=4*length(sets), by=4)+0.3, labels=asterisks, cex=1.2, col="darkblue")
dev.off()
