...
load("../data/GEMM_MEKi_RNAseq.RData")
...
treatment <- eset$Treatment
treatment[treatment %in% "control"] <- "untreated"
colannot <- data.frame(Treatment = treatment, Subtype = subtype)
...
hc <- HeatmapAnnotation(df=colannot[col.order,,drop=FALSE],
										col=list(Treatment=c("untreated"="gold", "MEKi"="darksalmon"),
													Subtype=c("MUC"="cyan3", "PRO"="mediumpurple2", "unclassified"="gray")),
...
pdf("../figures/Fig4_GEMM_MEKi_SubtypeHeatmap.pdf", width=6, height=6)
