...
metadata <- read.table("../data/TCGA_KRASwt_subtypes.tsv", sep="\t", header=TRUE, check.names=FALSE)
...
for (set in sets) {
	...
	pvalues <- c(pvalues, kruskal.test(metadata[,set] ~ factor(metadata$Subtype))$p.value)
}
names(pvalues) <- sets
...
pdf("../figures/FigS3_TCGAwt_HallmarkSignatureBarplots.pdf", width=4.2, height=7.5)
...
boxplot(scores ~ subtypes, horizontal=TRUE, col=colors, outline=FALSE,
				...
				xlab="Signature scores", yaxt="n", main="TCGA - KRAS wildtype")
axis(side=2, at=seq(from=2, to=4*length(sets), by=4), labels=rep("", length(sets)), las=2, cex.axis=0.7)
text(x=1.6, y=seq(from=2, to=4*length(sets), by=4)+0.3, labels=asterisks, cex=1.2, col="darkblue")
dev.off()
