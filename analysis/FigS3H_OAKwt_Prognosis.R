...
p <- ggsurvplot(fit, 
	data = data,
   title = paste0("OAK - KRAS wildtype (n=",nbTumors,", docetaxel)"),
...
p$plot <- p$plot + ggplot2::annotate("text", x=median.1+0.5, y=0.54, label=median.1, color="cyan3", size=6)
p$plot <- p$plot + ggplot2::annotate("text", x=median.2-0.5, y=0.46, label=median.2, color="mediumpurple2", size=6)
p$plot <- p$plot + ggplot2::annotate("text", x=median.3+0.3, y=0.6, label=median.3, color="green3", size=6)
p$plot <- p$plot + theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank(),
							panel.background = element_blank(), axis.line = element_line(colour = "black"))

pdf("../figures/FigS3_OAKwt_SurvivalBySubtype_Docetaxel.pdf", width=6, height=6, onefile=FALSE)
p
dev.off()
